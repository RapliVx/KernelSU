...
fun getBugreportFile(context: Context): File {
    ...
    val ksuDir = if (me.weishu.kernelsu.Natives.isAPatchInstalled) "/data/adb/ap" else "/data/adb/ksu"
    
    shell.newJob().add("tar -czf ${bootlogFile.absolutePath} -C $ksuDir/log .").exec()
    ...
    shell.newJob().add("du -sh $ksuDir/* > ${ksuFileSize.absolutePath}").exec()
    shell.newJob().add("cp /data/system/packages.list ${appListFile.absolutePath}").exec()
    shell.newJob().add("getprop > ${propFile.absolutePath}").exec()
    shell.newJob().add("cp $ksuDir/.allowlist ${allowListFile.absolutePath}").exec()
